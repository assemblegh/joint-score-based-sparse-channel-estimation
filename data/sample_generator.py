...
class sample_generator(object):
    ...
    def QAM_const(self):
        sqrt_mod_n = int(np.sqrt(self.mod_n))
    ...
    def QAM_N_ind(self, NT, batch_size):
        indices_QAM = torch.randint(low=0, high=(int(np.sqrt(self.mod_n))), size=(batch_size, 2*NT))
        return indices_QAM
    ...
    def joint_indices(self, indices):
        ...
        joint_indices = int(np.sqrt(self.mod_n))*real_part + complex_part
